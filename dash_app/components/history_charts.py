...
def fig_cancel_timing_heatmap(g: pd.DataFrame, dim: str = "stay", metric: str = "rate",
    ...
    (0 = arrival day … max_day, plus a 'max_day+1 plus' overflow), rows = stay segments
    or lead buckets. `metric='rate'` colours by the per-cell cancel rate (share of the
    segment's bookings), `metric='count'` by the raw number of cancellations. Both are
    always in the hover. Equal-width day columns — no misleading bucket widths."""
    ...
    piv_n = g.pivot(index="row", columns="day", values="n_cancel").reindex(index=rows, columns=day_labels)
    rate = piv_rate.to_numpy(dtype="float64") * 100
    n = piv_n.to_numpy(dtype="float64")

    if metric == "count":
        z = n
        text = [["" if (np.isnan(v) or v == 0) else f"{int(v):,}" for v in r] for r in n]
        colorscale = [[0.0, "#F3F1EA"], [1.0, theme.PURPLE]]
        cbar = "Cancellations"
    else:
        z = rate
        text = [["" if np.isnan(v) else f"{v:.1f}%" for v in r] for r in rate]
        colorscale = theme.CANCEL_SCALE
        cbar = "Cancel %"
    custom = np.dstack([np.nan_to_num(n), np.nan_to_num(rate)])
    fig = go.Figure(go.Heatmap(
        z=z, x=day_labels, y=rows, customdata=custom, colorscale=colorscale,
        xgap=2, ygap=2, hoverongaps=False, text=text, texttemplate="%{text}",
        textfont=dict(size=10, color="#20140E"),
        colorbar=dict(title=cbar, thickness=12),
        hovertemplate="<b>%{y}</b> · %{x} days before arrival"
                      "<br>Cancellations: %{customdata[0]:,.0f}"
                      "<br>Cancel rate (of segment): %{customdata[1]:.2f}%<extra></extra>"))
